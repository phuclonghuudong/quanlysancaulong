...
public final class SanPham extends JPanel implements ActionListener, ItemListener {
    ...
    public ArrayList<SanPhamDTO> listDS = spBUS.getAllLoaiSanPham();
    private TableModel<SanPhamDTO> tableModel;
    JFrame owner = (JFrame) SwingUtilities.getWindowAncestor(this);
    IntegratedSearch search;
    DefaultTableModel tblModel;

    String[] header = new String[]{"Mã SP", "Tên SP", "Loại Hàng", "Giá SP", "Số lượng", "Đơn vị", "Ghi chú", "Trạng thái"};
    String[] headerSearch = new String[]{"Tất cả", "Mã sản phẩm", "Tên sản phẩm", "Loại sản phẩm", "Trạng thái"};
    ...
    public void initComponent() {
        ...
        String[] methodNames = {"getMasanpham", "getTensanpham", "getTenloaisanpham", "getGiaban", "getSoluong", "getDonvi", "getGhichu", "getTrangthai"};
        ...
        search.btnReset.addActionListener((ActionEvent e) -> {
            ...
            listDS = spBUS.getAllLoaiSanPham();
            loadDataTable(listDS);
        });
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == mainFunction.btn.get("create")) {
            SanPhamDialog lsDialog = new SanPhamDialog(this, owner, "Thêm sản phẩm", true, "create");
        } else if (e.getSource() == mainFunction.btn.get("update")) {
            int index = getRowSelected();
            if (index != -1) {
                SanPhamDialog lsDialog = new SanPhamDialog(this, owner, "Chỉnh sửa sản phẩm", true, "update", listDS.get(index));
            }
        } else if (e.getSource() == mainFunction.btn.get("delete")) {
            int index = getRowSelected();
            if (index != -1) {
                int input = JOptionPane.showConfirmDialog(null,
                        "Bạn có chắc chắn muốn xóa loại sản phẩm này?", "Xóa sản phẩm",
                        JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE);
                if (input == 0) {
                    boolean deleted = spBUS.delete(listDS.get(index));
                    if (deleted) {
                        listDS = spBUS.getAllLoaiSanPham();
                        loadDataTable(listDS);
                    } else {
            ...
            int index = getRowSelected();
            if (index != -1) {
                SanPhamDialog lsDialog = new SanPhamDialog(this, owner, "Xem sản phẩm", true, "view", listDS.get(index));
            }
